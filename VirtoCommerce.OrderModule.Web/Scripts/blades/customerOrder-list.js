angular.module('virtoCommerce.orderModule')
    .controller('virtoCommerce.orderModule.customerOrderListController', ['$scope', '$localStorage', 'virtoCommerce.orderModule.order_res_customerOrders', 'platformWebApp.bladeUtils', 'platformWebApp.dialogService', 'platformWebApp.authService', 'uiGridConstants', 'platformWebApp.uiGridHelper', 'platformWebApp.ui-grid.extension', 'virtoCommerce.orderModule.knownOperations', 'virtoCommerce.orderModule.workflows', 'platformWebApp.accounts', 'platformWebApp.authDataStorage',
        function ($scope, $localStorage, customerOrders, bladeUtils, dialogService, authService, uiGridConstants, uiGridHelper, gridOptionExtension, knownOperations, workflows, accounts, authDataStorage) {
            var blade = $scope.blade;
            var bladeNavigationService = bladeUtils.bladeNavigationService;
            var organizationId;
            $scope.uiGridConstants = uiGridConstants;

            accounts.get({ id: authDataStorage.getStoredData().userName }, function (account) {
                if (account.memberId) {
                    workflows.getOrganizationsByMemberId({ id: account.memberId }, function (organization) {
                        if (organization.length) organizationId = organization[0].id;
                    }, function (error) {
            ...
            function showFilterDetailBlade(bladeData) {
                ...
                if (organizationId) angular.extend(newBlade, { organizationId: organizationId });
                bladeNavigationService.showBlade(newBlade, blade);
            }
